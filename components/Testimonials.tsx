...
/* The two classes the markup below reaches for — `scroll-reel-char` on every
 * character and `scroll-reel-exit` on the outgoing block — have no definition
 * anywhere else, so they live here. Injected in-component rather than from
 * globals.css because that's how Footer and ZoomInScroll carry their keyframes.
 *
 * The exit duration is EXIT_MS: the old block finishes leaving exactly as the
 * new one mounts. Killing the char animation inside an exiting block is what
 * the Chars comment refers to — without it, characters mid-rise would keep
 * rising while their parent fades out. */
const STYLES = `
...
@keyframes scroll-reel-block-exit {
...
.scroll-reel-char {
  ...
  white-space: pre;
  animation: scroll-reel-char-rise 520ms cubic-bezier(0.22, 1, 0.36, 1) both;
}

.scroll-reel-exit {
  animation: scroll-reel-block-exit ${EXIT_MS}ms ${EASE_INOUT} both;
}
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    ...
    >
      ...
      />
      {/* diagonal gradient sheen — clay and cream, to match the rest of the
          site; this arrived as a violet/blue sweep that read as another brand */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-[3] blur-[6px] mix-blend-overlay"
        style={{
          background:
            "linear-gradient(220.99deg, rgba(138,111,89,0) 32%, rgb(138,111,89) 41%, rgb(215,195,177) 47%, rgba(247,245,241,0.57) 54%, rgba(247,245,241,0) 65%)",
        }}
      />
    </div>
  );
