...
const LINE_BASE = { r: 15, g: 15, b: 15, a: 0.18 };
...
export default function KineticGrid({
    ...
    const getWarpedPoint = useCallback(
        (
            ...
            const proximity = Math.max(0.08, (1 - dist / INFLUENCE_RADIUS) * pinFactor);
    ...
    const draw = useCallback(
        (now: number) => {
            ...
            const theme = {
                default: {
                    bg: "#f8f7f5",
                    lineActive: { r: 20, g: 20, b: 20, a: 0.85 },
                ...
                monochrome: {
                    bg: "#ffffff",
                    lineActive: { r: 15, g: 15, b: 15, a: 0.85 },
                    nodeActive: { r: 15, g: 15, b: 15, a: 1.0 },
                    glow: "15,15,15",
                    ripple: "15,15,15",
                },
            }[globalColor ?? "default"];
            ...
            const drawSeg = (p1: Point, p2: Point, pr1: number, pr2: number) => {
                const avg = (pr1 + pr2) / 2;
                const t = Math.max(0.08, avg * avg * (3 - 2 * avg));
                ctx.beginPath();
            ...
            for (let row = 0; row < rows; row++) {
                for (let col = 0; col < cols; col++) {
                    ...
                    const t = Math.max(0.08, pr * pr * (3 - 2 * pr));
