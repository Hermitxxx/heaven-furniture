...
export interface FlippingWordSwapProps {
  ...
  /** Renders an anchor instead of a button. Use for navigation. */
  href?: string;
  /** Click handler, on whichever element `href` selects. */
  onClick?: (event: ReactMouseEvent<HTMLElement>) => void;
  /** Forwarded to the rendered element, for scroll-spy "current page" state. */
...
export function FlippingWordSwap({
  ...
  // Anchor when it navigates, button when it toggles. `aria-pressed` is toggle
  // semantics and would be wrong on a link, so the anchor carries `aria-current`
  // instead and lets the scroll spy say which section is showing.
  if (href !== undefined) {
    return (
      <a {...shared} href={href} aria-label={word1} aria-current={ariaCurrent}>
        {body}
      </a>
    );
  }
