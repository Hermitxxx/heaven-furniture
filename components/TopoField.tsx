...
export type TopoFieldProps = {
  ...
  density?: number;
  /** Draws the faint 48px rule grid behind the contour lines. */
  grid?: boolean;
  opacity?: number;
  hue?: number;
  saturation?: number;
...
function patchTopoField(
  ...
  if (!grid) {
    // Both strengths above are written as a `* <n>` tail on the same clamp, so
    // zeroing the result drops the grid pass in either mode and leaves the
    // contour pass untouched. The fract/step work feeding it is then unread and
    // the shader compiler strips it.
    next = next.replace(
      /gridLines = clamp\(gridLines, 0\.0, 1\.0\) \* [\d.]+;/,
      "gridLines = 0.0;",
    );
  }
  return next;
...
/**
 * TopoField — an isolated, full-bleed animated WebGL background: ultra-thin contour
 * ("topographic") lines drawn from 2D simplex noise, drifting slowly over time, optionally over a
 * faint 48px rule grid (`grid`). Ships as a sandboxed iframe (matching the source's isolation
 * technique) so the shader's own script/canvas loop stays completely self-contained.
 */
export default function TopoField({
  ...
  const source = useMemo(
    ...
    [resolvedMode, safeLength, safeDensity, grid],
  );
